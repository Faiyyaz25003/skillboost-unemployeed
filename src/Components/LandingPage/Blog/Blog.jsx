...
articlesByCategory["All"] = [
  articlesByCategory["Career Tips"][0],
  articlesByCategory["Digital Skills"][0],
  articlesByCategory["Industry Insights"][0],
  articlesByCategory["Success Stories"][0],
  articlesByCategory["Guides"][0],
  articlesByCategory["Career Tips"][2],
];

// Flat lookup so any card (including sidebar "Popular Posts") can open the
// same rich modal, keyed by article title.
const articleByTitle = Object.fromEntries(
  [
    ...articlesByCategory["Career Tips"],
    ...articlesByCategory["Digital Skills"],
    ...articlesByCategory["Industry Insights"],
    ...articlesByCategory["Success Stories"],
    ...articlesByCategory["Guides"],
  ].map((a) => [a.title, a]),
);
...
const popularPosts = [
  {
    ...
    matchTitle: "Top 10 In-Demand Skills to Boost Your Career in 2025",
  },
  {
    ...
    matchTitle: "Full Stack Developer Roadmap for Beginners in 2025",
  },
  {
    ...
    matchTitle: "How Rahul Landed His Dream Job as a Software Developer",
  },
  {
    ...
    matchTitle: "Top 10 Tech Trends Shaping the Future of Work in 2025",
  },
];

/* ------------------------------------------------------------------ */
/* Helper: turn a short excerpt into a fuller article body for the      */
/* modal, tailored a little by category/role so it doesn't feel like    */
/* one generic block repeated everywhere.                               */
/* ------------------------------------------------------------------ */
function buildBody(article) {
  const paragraphs = [article.excerpt];

  paragraphs.push(
    `Drawing on ${article.author}'s experience as a ${article.role}, this piece walks through what actually works in practice, not just the theory — with examples you can apply the same week.`,
  );

  if (article._category === "Success Stories") {
    paragraphs.push(
      `Like every real journey, the path wasn't a straight line. ${article.author.split(" ")[0]} shares the setbacks along the way, and the specific habits that turned things around.`,
    );
  } else {
    paragraphs.push(
      `Start small: pick one idea from this article, apply it this week, and build from there. Consistency beats intensity when it comes to career growth.`,
    );
  }
...
/* ------------------------------------------------------------------ */
/* Post Modal — used for both grid articles and popular posts          */
/* ------------------------------------------------------------------ */
function PostModal({ article, onClose }) {
  ...
  return (
    ...
    >
      <div className="relative max-h-[92vh] w-full overflow-y-auto rounded-t-2xl bg-white shadow-2xl sm:max-h-[88vh] sm:max-w-2xl sm:rounded-2xl">
        {/* Close button */}
        <button
          onClick={onClose}
          aria-label="Close"
        ...
        <div className="relative h-52 w-full shrink-0 sm:h-64">
          ...
          <span
            className={`absolute left-4 top-4 rounded-md px-2.5 py-1 text-xs font-medium text-white ${tagColor}`}
          >
            {article._category}
          </span>
          {article.quote && (
            <p className="absolute bottom-4 left-4 right-14 flex items-start gap-2 text-base font-semibold leading-snug text-white sm:text-lg">
              <Quote className="h-5 w-5 shrink-0 text-yellow-400" />
              {article.quote}
            </p>
          )}
        </div>
        ...
        <div className="p-5 sm:p-8">
          ...
          <h2 className="mt-3 text-xl font-extrabold leading-snug text-slate-900 sm:text-2xl">
            {article.title}
          </h2>
          ...
          <div className="mt-4 flex items-center gap-3 border-y border-slate-100 py-4">
            ...
            <div className="leading-tight">
              ...
              <p className="text-xs text-slate-400">{article.role}</p>
            </div>
            <button
              aria-label="Bookmark this post"
              className="ml-auto flex h-9 w-9 items-center justify-center rounded-full text-slate-300 transition hover:bg-blue-50 hover:text-blue-700"
            >
          ...
          <button
            onClick={onClose}
            className="mt-8 w-full rounded-lg bg-blue-700 py-3 text-sm font-medium text-white transition hover:bg-blue-800 sm:w-auto sm:px-6"
          >
            Close
          </button>
        </div>
...
function ArticleCard({ article, onOpen }) {
  ...
  return (
    <article
      ...
      className="group flex cursor-pointer flex-col overflow-hidden rounded-xl border border-slate-100 bg-white shadow-sm transition hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
    >
...
/* ------------------------------------------------------------------ */
function SuccessStoryCard({ article, onOpen }) {
  return (
    <article
      role="button"
      tabIndex={0}
...
export default function BlogPage() {
  ...
  const openPopularPost = (post) => {
    const match = articleByTitle[post.matchTitle];
    if (match) {
      setSelectedArticle(match);
    } else {
      // fallback: show whatever minimal info we have
      setSelectedArticle({
        title: post.title,
        date: post.date,
        image: post.image,
  ...
  return (
    <main className="bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-50 via-blue-50 to-indigo-50">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-4 py-10 sm:gap-10 sm:px-6 sm:py-14 md:grid-cols-2 md:py-20">
          <div>
            <h1 className="mt-2 text-2xl font-extrabold leading-tight text-slate-900 xs:text-3xl sm:mt-5 sm:text-4xl md:text-5xl">
            ...
            <div className="mt-6 flex max-w-md flex-col gap-2 rounded-xl bg-white p-1.5 shadow-md sm:mt-8 sm:flex-row sm:items-center">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        {/* Filter Tabs */}
        <div className="mb-8 -mx-4 flex gap-2 overflow-x-auto px-4 pb-1 sm:mb-10 sm:mx-0 sm:flex-wrap sm:justify-start sm:overflow-visible sm:px-0">
          {filters.map(({ name, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition sm:px-4 ${
                  isActive
            ...
            <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
              ...
              <ul className="space-y-4">
                {popularPosts.map((p) => (
                  <li key={p.title}>
                    <button
                      onClick={() => openPopularPost(p)}
                      className="flex w-full gap-3 rounded-lg text-left transition hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                    >
                      <img
                        src={p.image}
                        alt={p.title}
        ...
        {/* CTA Banner */}
        <div className="relative mt-12 flex flex-col items-center justify-between gap-6 overflow-hidden rounded-2xl bg-gradient-to-r from-blue-900 to-blue-700 px-5 py-7 text-center sm:mt-14 sm:px-8 sm:py-8 sm:text-left md:flex-row">
          ...
          <button className="flex w-full shrink-0 items-center justify-center gap-2 rounded-lg border border-white/70 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-white hover:text-blue-800 sm:w-auto">
            Write for Us
